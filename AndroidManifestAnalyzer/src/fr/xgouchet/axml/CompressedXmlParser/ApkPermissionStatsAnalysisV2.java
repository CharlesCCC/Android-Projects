...
public class ApkPermissionStatsAnalysisV2 {
	public static void main(String[] args) throws IOException {
		...
		writeToCSV("TotalPermissionCount", permissionCount);

		System.out.println("---------------------------------------");

		appPermissionCount = sortHashMapByValues(appPermissionCount);
		writeToCSV("AppPermissionCount", appPermissionCount);
	...
	private static void writeToCSV(String fileName, HashMap<String, Integer> inputStream){
		try
		{
			String filename= fileName + ".csv";
			FileWriter fw = new FileWriter(filename,true); //the true will append the new data
			for(String s : inputStream.keySet()){
				fw.write("Key:" + s + ", Value:," + inputStream.get(s) + " \n");
				System.out.println("Key:" + s + ", Value:," + inputStream.get(s));
			}
